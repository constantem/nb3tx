...
$jscomp.polyfill = function (c, f, g, l) {
	if (f) {
		...
		l = g[c];
		f = f(l);
		f != l && null != f && $jscomp.defineProperty(g, c, {
			configurable: !0,
			writable: !0,
			value: f
		})
...
(function (c) {
	...
	var h = c.fn.dataTable,
		n = 0,
		m = function (a, b) {
			if (!(this instanceof m)) throw "FixedHeader must be initialised with the 'new' keyword.";
			!0 === b && (b = {});
			a = new h.Api(a);
	...
	c.extend(m.prototype, {
		...
		_constructor: function () {
			var a = this,
				b = this.s.dt;
			c(f).on("scroll" + this.s.namespace,
				function () {
					a._scroll()
				}).on("resize" + this.s.namespace, h.util.throttle(function () {
		...
		_horizontal: function (a, b) {
			var c = this.dom[a],
				d = this.s.position,
				k = this.s.scrollLeft;
			c.floating && k[a] !== b && (c.floating.css("left", d.left - b), k[a] = b)
		},
		_modeChange: function (a, b, e) {
			...
			h && h.blur();
			"in-place" === a ? (d.placeholder && (d.placeholder.remove(), d.placeholder = null), this._unsize(b), "header" === b ? d.host.prepend(f) : d.host.append(f), d.floating && (d.floating.remove(), d.floating =
				...
				k.width + "px"));
			h && h !== g.activeElement && setTimeout(function () {
				h.focus()
			}, 10);
		...
		_positions: function () {
			...
			k.length ? (b.tfootTop =
				k.offset().top, b.tfootBottom = b.tfootTop + k.outerHeight(), b.tfootHeight = b.tfootBottom - b.tfootTop) : (b.tfootTop = b.tbodyTop + e.outerHeight(), b.tfootBottom = b.tfootTop, b.tfootHeight = b.tfootTop)
		},
		_scroll: function (a) {
			var b = c(g).scrollTop(),
				e = c(g).scrollLeft(),
			...
			this.c.footer && this.dom.tfoot.length && (this.s.enable && (k = !d.visible || b + d.windowHeight >= d.tfootBottom + this.c.footerOffset ? "in-place" : d.windowHeight + b > d.tbodyTop + d.tfootHeight + this.c.footerOffset ? "in" : "above"), (a || k !== this.s.footerMode) && this._modeChange(k, "footer", a), this._horizontal("footer", e))
		}
	});
	m.version = "3.1.6";
	m.defaults = {
	...
	c(g).on("init.dt.dtfh", function (a, b, e) {
		"dt" === a.namespace &&
			(a = b.oInit.fixedHeader, e = h.defaults.fixedHeader, !a && !e || b._fixedHeader || (e = c.extend({}, e, a), !1 !== a && new m(b, e)))
	});
	...
	c.each(["header", "footer"], function (a, b) {
		h.Api.register("fixedHeader." + b + "Offset()", function (a) {
			var c = this.context;
			return a === l ? c.length && c[0]._fixedHeader ? c[0]._fixedHeader[b + "Offset"]() : l : this.iterator("table", function (c) {
