    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $("#btnConfirm").click(function() {
                console.log(">>> 開始執行表單檢查...");

                var fromAccount = $("#fromAccountSelect").val();
                var fromCurr = $("select[name='fromCurr']").val();
                var toAccount = $("select[name='toAccount']").val();
                var toCurr = $("select[name='toCurr']").val();
                var amount = $("input[name='amount']").val();

                if (fromAccount == "" || fromCurr == "" || toAccount == "" || toCurr == "" || amount == "") {
                    errorBlock('錯誤', null, ['請完成所有必填欄位'], '確定', null);
                    return;
                }

                if (fromAccount === toAccount) {
                    errorBlock('錯誤', null, ['轉出入帳號不可相同'], '確定', null);
                    return; 
                }
                
                var isFromTWD = (fromCurr === "TWD");
                var isToTWD = (toCurr === "TWD");

                if ((isFromTWD && isToTWD) || (!isFromTWD && !isToTWD)) {
                    errorBlock('錯誤', null, ['不可同時為台幣或同時為外幣'], '確定', null);
                    return; 
                }

                console.log(">>> 檢查通過，送出表單！");
                $("form").submit(); 
            });
            ...
            $("#btnReset").click(function() {
                ...
                $("#fromAccountSelect").val("");
                $("select[name='fromCurr']").val("");
                $("select[name='toAccount']").val("");
                $("select[name='toCurr']").val("");
            ...
            <section id="main-content" class="container">
                        ...
                        <div class="ttb-input-block tab-pane fade show active" id="nav-trans-now" role="tabpanel" aria-labelledby="nav-home-tab">
                            ...
                            <form action="${pageContext.request.contextPath}/ForeignExchangeTransfer/step3-confirm" method="post">
                                ...
                                <div class="row justify-content-center" style="margin-top: 20px; width: 100%;">
                                    <input type="button" id="btnReset" class="ttb-button btn-flat-gray" value="重新輸入" style="margin-right: 15px;">
                                    <button type="button" id="btnConfirm" class="ttb-button btn-flat-orange">確定</button>
                                </div>
                ...
                <ol class="description-list list-decimal">
                    <p>說明</p>
                        ...
                        <li><span><font color="red">併行作業期間:倘您已曾經登入新版網路銀行後，前於舊版網銀已預約之轉帳交易，統一將移轉至新版網路銀行執行及查詢;另新、舊版網路銀行執行預約交易，需回原執行之網路銀行查詢，無法於兩系統間相互查詢。</font></span></li></ol>
            </section>
        </main>
    </div>
